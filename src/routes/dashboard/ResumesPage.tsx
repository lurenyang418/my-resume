...
import { isResumeData, sanitizeResumeData } from "@/lib/resumeData";
...
const ResumeWorkbench = () => {
  ...
  useEffect(() => {
    const syncResumesFromFiles = async () => {
      try {
        ...
        const loadedResumes = new Map<string, ReturnType<typeof sanitizeResumeData>>();
        ...
        for await (const entry of dirHandle.values()) {
          if (entry.kind === "file" && entry.name.endsWith(".json")) {
            try {
              ...
              const resumeData: unknown = JSON.parse(content);
              if (isResumeData(resumeData)) {
                const safeResume = sanitizeResumeData(resumeData);
                const current = loadedResumes.get(safeResume.id);
                if (
                  !current ||
                  Date.parse(safeResume.updatedAt) >= Date.parse(current.updatedAt)
                ) {
                  loadedResumes.set(safeResume.id, safeResume);
                }
              }
            } catch (error) {
        ...
        loadedResumes.forEach((resume) => updateResumeFromFile(resume));
      } catch (error) {
  ...
  const handleImportJson = () => {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        if (!isResumeData(newResume)) throw new Error("Invalid resume schema");
        addResume(sanitizeResumeData(newResume));
  ...
  return (
    <motion.div
      ...
      className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6 sm:py-10 lg:px-8"
    >
      <motion.div
          ...
          <Alert className="mb-6 bg-green-50/50 dark:bg-green-950/30 border-green-200 dark:border-green-900">
            <AlertDescription className="flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <Button
                size="sm"
                variant="outline"
                className="w-full hover:bg-green-100 sm:ml-4 sm:w-auto dark:hover:bg-green-900"
                onClick={() => {
          ...
          >
            ...
            <AlertDescription className="flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <Button
                variant="outline"
                size="sm"
                className="w-full hover:bg-red-100 sm:ml-4 sm:w-auto dark:hover:bg-red-900"
                onClick={() => {
                  navigate("/app/dashboard/settings");
                }}
              >
        ...
        <div className="grid grid-cols-2 gap-2 sm:flex sm:items-center">
          ...
          >
            <Button
              ...
              variant="outline"
              className="w-full hover:bg-gray-100 dark:border-primary/50 dark:hover:bg-primary/10"
            >
          ...
          >
            <Button
              ...
              variant="default"
              className="w-full bg-gray-900 text-white hover:bg-gray-800 dark:bg-primary dark:text-primary-foreground dark:hover:bg-primary/90"
            >
