...
const TemplatesPage = () => {
  ...
  const handleCreateResume = (templateId: string) => {
    ...
    navigate(`/app/workbench/${resumeId}`);
  };
    ...
    <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-10 lg:px-8">
      <div className="flex flex-col space-y-8">
        ...
        <motion.div
          ...
          initial="hidden"
          animate="show"
          className="grid min-w-0 grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4"
        >
          {DEFAULT_TEMPLATES.map((template) => {
            ...
            return (
              <motion.div key={template.id} variants={item}>
                <Card
                  className={cn(
                    "group mx-auto w-full max-w-sm cursor-pointer overflow-hidden transition-all hover:-translate-y-1 hover:shadow-xl",
        ...
        >
          {previewTemplate && (
            <DialogContent className="w-[calc(100vw-2rem)] max-w-[680px] overflow-hidden rounded-xl border-0 bg-white p-0 shadow-lg dark:bg-gray-900">
