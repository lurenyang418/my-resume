  ...
  const templateSheetIndex = childrenArray.findIndex((child) => {
    if (React.isValidElement<{ children: React.ReactNode }>(child)) {
      ...
      if (React.isValidElement<{ children: React.ReactNode }>(tooltip)) {
        const children = tooltip.props.children;
        const trigger = (Array.isArray(children) ? children : [children]).find(
          (c: any) => c?.type?.name === "TooltipTrigger"
        );
        if (trigger && React.isValidElement<{ children: React.ReactNode }>(trigger)) {
          ...
          if (React.isValidElement<{ children: React.ReactNode }>(content)) {
            ...
            return (
              React.isValidElement(icon) && typeof icon.type !== "string" && (icon.type as React.ComponentType).displayName === "TemplateSheet"
            );
          }
        }
      }
